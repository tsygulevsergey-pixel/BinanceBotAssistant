...
## October 16, 2025: Action Price Fix - Confirm Buffer (EMA200 Touch Protection)

**CRITICAL FIX: Буфер безопасности для подтверждения**

**Проблема:**
- EMA200 бота отличается от Binance на ~0.24% (бот: 13.636, Binance: 13.670)
- Свеча с Low=13.648 проходила проверку (13.648 > 13.636), но на Binance КАСАЛАСЬ EMA200 (13.648 < 13.670)
- Ложные сигналы из-за расхождения в расчёте EMA200

**Корневая причина:**
- pandas-ta и Binance могут считать EMA200 по-разному (разные начальные точки, история данных)
- Даже 0.24% разница = ложные сигналы на пограничных случаях

**Исправление:**
1. **Буфер в конфиге**: `confirm_buffer_atr: 0.1` (10% ATR)
2. **LONG**: Low > EMA200 + (ATR × 0.1)
3. **SHORT**: High < EMA200 - (ATR × 0.1)
4. **Детальное логирование**: Показывает EMA200, буфер, и требуемый уровень

**Пример:**
```
EMA200: 13.637, ATR: 0.211
Буфер: 0.211 × 0.1 = 0.021
Требуемый Low: 13.637 + 0.021 = 13.658

Low свечи: 13.648 < 13.658 → ❌ ОТКЛОНЁН (правильно!)
```

**Файлы:**
- `config.yaml` (строка 388): параметр `confirm_buffer_atr`
- `src/action_price/engine.py` (строки 62, 297-364): реализация буфера + логирование
