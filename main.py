    ...
    def _save_gluk_signal(self, gluk_signal: Dict) -> bool:
        ...
        try:
            signal = GlukSignal(
                ...
                symbol=gluk_signal['symbol'],
                pattern_type=gluk_signal.get('pattern_type', 'gluk_body_cross'),
                timeframe=gluk_signal['timeframe'],
                direction=gluk_signal['direction'],
                zone_id=gluk_signal.get('zone_id', 'gluk_ema200'),
                zone_low=float(gluk_signal.get('zone_low', gluk_signal['stop_loss'])),
                zone_high=float(gluk_signal.get('zone_high', gluk_signal['entry_price'])),
                ...
                confidence_score=float(gluk_signal.get('confidence_score', 5.0)),
                confluence_flags=gluk_signal.get('confluence_flags', {}),
                market_regime=gluk_signal.get('market_regime', 'EXPERIMENTAL'),
                status='ACTIVE',
